                                         ...
                                         unsigned short dstPort)
{
    ...
    if(verbose)
    {
        ...
        log << ", Port = " << this->tcpudpReceivePorts[this->activeTCPUDPReceiveSocketIndex];
        log << ", RSID = " << this->tcpudpReceiveSockets[this->activeTCPUDPReceiveSocketIndex] << "]\n";
        ...
        log << "Source port: " << srcPort_16 << "\n";
        log << "Destination port: " << dstPort_16 << "\n";
        log << "Sequence number: " << ntohl(tcp->seq) << "\n";
        log << "ACK sequence number: " << ntohl(tcp->ack_seq) << "\n";
        log << "Window size: " << ntohs(tcp->window) << "\n";
    }
    ...
    while(1)
    {
        ...
        if(selectResult > 0)
        {
            ...
            {
                if(verbose)
                    log << "Received packet is neither ICMP neither TCP. Continue receiving...\n";
                continue;
            }
            ...
            else if(IPProtocol == IPPROTO_TCP)
            {
                ...
                {
                    log << "coming from the right target!\n";
                ...
                {
                    log << "not coming from the target. Continue receiving...\n";
                }
            }
        }
