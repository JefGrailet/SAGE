...
{
    bool timestampRequest = usingTimestampRequests;
    ...
    uint32_t UTTimeSinceMidnight = 0; // For saving in a ProbeRecord later
    if(timestampRequest)
    {
        // Main fields
    ...
    if(verbose)
    {
        ...
        log << ", RSID = " << this->icmpReceiveSocketRAW << "]\n";
    
        // Actual details on the probe.
        log << "ICMP probing:\n";
        log << "Source address: " << src << "\n";
        log << "Destination address: " << dst << "\n";
        log << "IP identifier (source): " << IPIdentifier << "\n";
        log << "Initial TTL: " << (int) TTL << "\n";
        log << "ICMP identifier: " << ICMPidentifier << "\n";
        log << "ICMP sequence: " << ICMPsequence << "\n";
    }
    ...
    if(verbose)
    {
        log << "Started listening for a reply...\n";
    }
    ...
    while(1)
    {
        ...
        if(selectResult > 0)
        {
            ...
            /**
             * recvfrom method reads an entire packet into the buffer for SOCK_RAW,SOCK_DGRAM, and 
             * SOCK_SEQPACKET types of sockets, if the buffer is less than the packet length the 
             * rest of the packet is overflowing part is discarded. However, for SOCK_STREAM it 
             * reads in the portions of a packet as soon as it becomes available.
             *
             * Since our socket is SOCK_RAW, we can freely assume that the packet is received as a 
             * whole without worrying about packet portions received. However, though our socket 
            ...
            {
                if(verbose)
                    log << "Received packet is not IPv4. Continue receiving...\n";
                continue;
            }
            ...
            {
                if(verbose)
                    log << "Received less bytes than minimum header length. Continue receiving...\n";
                continue;
            }
            ...
            {
                if(verbose)
                    log << "Received less bytes than announced in header. Continue receiving...\n";
                continue;
            }
            ...
            {
                if(verbose)
                    log << "Error while re-computing IP header checksum. Continue receiving...\n";
                continue;
            }
            ...
            {
                if(verbose)
                    log << "Received packet is not an ICMP one. Continue receiving...\n";
                continue;
            }
            ...
            {
                if(verbose)
                    log << "Error while re-computing ICMP header checksum. Continue receiving...\n";
                continue;
            }
            ...
            if(icmp->type == DirectProber::ICMP_TYPE_TIME_EXCEEDED || icmp->type == DirectProber::ICMP_TYPE_DESTINATION_UNREACHABLE)
            {
                   ...
                   ntohs((payloadicmp->un).echo.sequence) == ICMPsequence_16)
                {
                    ...
                    ProbeRecord newRecord(dst, 
                                          rplyAddress, 
                                          REQTime, 
                                          TimeVal::getCurrentSystemTime(), 
                                          TTL, 
                                          ip->ip_ttl, 
                                          icmp->type, 
                                          icmp->code, 
            ...
            {
                if(verbose)
                    log << "An unknown packet has been received. Continue receiving...\n";
            }
        ...
        else if(selectResult == 0)
        {
            ...
            return ProbeRecord(dst, InetAddress(0), REQTime, TimeVal::getCurrentSystemTime(), TTL, 0, 255, 255, IPIdentifier, 0, 0, 0, 0, 0, 0, 1, usingFixedFlowID);
        }
        ...
        else
        {
            ...
            return DirectProber::VOID_RECORD;
        }
    }
}
