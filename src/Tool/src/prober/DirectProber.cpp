 ...
 * -October 2019: removal of all "new" and "delete" operations with the exception of two arrays. 
 *  ProbeRecord objects are no longer dynamically allocated.
 */
...
#include <set>
using std::set;
#include <sys/time.h> // For gettimeofday (by J.-F. Grailet)
...
ProbeRecord DirectProber::VOID_RECORD = ProbeRecord();
...
nbSuccessfulProbes(0)
{
    ...
    else
    {
        if(verbose)
        {
            log << "Sending raw socket has been created, socket identifier is ";
            log << sendSocketRAW << ".\n";
        }
    }
    ...
    else
    {
        ...
        else
        {
            if(verbose)
            {
                log << "ICMP receiving raw socket has been created, socket identifier is ";
                log << icmpReceiveSocketRAW << ".\n";
            }
        }
    }
                ...
                else
                {
                    ...
                    if(verbose)
                    {
                        if(probingProtocol == IPPROTO_TCP)
                            ...
                            log << "UDP ";
                        log << "receiving raw socket has been created on port ";
                        log << tcpudpReceivePorts[i] << ", socket identifier is ";
                        log << tcpudpReceiveSockets[i] << ".\n";
                    }
                    break;
                }
                i++;
...
{
    ProbeRecord firstICMPEchoReply = VOID_RECORD, rec = VOID_RECORD;
    ...
    do
    {
        ...
        else
        {
            ...
            {
                // Loop discovered
                firstICMPEchoReply = VOID_RECORD;
                rec = VOID_RECORD;
                break;
            }
            ...
            {
                firstICMPEchoReply = ProbeRecord(rec);
                rec = VOID_RECORD;
                break;
            }
    ...
    while(!stop && consecutiveAnonymousCount < MAX_CONSECUTIVE_ANONYMOUS_COUNT && firstICMPEchoReply.isVoidRecord() && 
          TTL <= (unsigned char) DirectProber::CONJECTURED_GLOBAL_INTERNET_DIAMETER);

    // Forward probing
    if(TTL == middleTTL && !firstICMPEchoReply.isVoidRecord())
    {
        TTL--;
        while(TTL > 1)
        {
    ...
    if(!firstICMPEchoReply.isVoidRecord())
