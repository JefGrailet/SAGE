 ...
 *      Author: jefgrailet
 *
 * Superclass modeling a single vertex of a network graph. It has a design similar to that of the 
 * Neighborhood class from the same module in SAGE v1, but differs in terms of interfaces and 
 * content. In particular, it manages its peers directly as Vertex objects rather than through 
...
public:
    ...
    inline list<Vertex*> *getPeers() { return &peers; }
     ...
     * map Peer -> Vertex. The reason why listing peers as Vertex objects is not done at Vertex 
     * construction is simply because it's simpler to first build all Vertex objects while putting 
     * peering info aside, then using the latter to complete the existing Vertex objects.
     */
    
    void setPeers(GraphPeers *peersInfo, map<Peer*, Vertex*> peersMap);
     ...
     * exists between this vertex and a given list of peer IPs, given as a list of Trail (for 
     * simplicity; said lists are normally the trail(s) used to identify other Vertex objects). 
    ...
    // Sorting method
    static bool smallerID(Vertex *v1, Vertex *v2);
...
protected:
    ...
    list<Trail> trails; // To identify the vertex by IP(s)
    ...
    // Edges ("out" only, i.e. which the tail is this vertex)
    list<Edge*> edges;
