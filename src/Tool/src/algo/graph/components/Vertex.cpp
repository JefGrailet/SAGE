 ...
 *      Author: jefgrailet
 *
 * Implements the class defined in Vertex.h (see this file to learn further about the goals 
 * of such a class).
 */
...
unsigned int Vertex::getSubnetCoverage()
{
...
void Vertex::setPeers(GraphPeers *peersInfo, map<Peer*, Vertex*> peersMap)
{
    peersOffset = peersInfo->offset;
    for(list<Peer*>::iterator i = peersInfo->peers.begin(); i != peersInfo->peers.end(); ++i)
    {
        map<Peer*, Vertex*>::iterator res = peersMap.find((*i));
        if(res != peersMap.end())
            peers.push_back(res->second);
    }
    
    /*
     * No sorting here, because Vertex objects are only numbered when the graph is finally 
...
Subnet* Vertex::getConnectingSubnet(InetAddress nextHop)
{
    for(list<Subnet*>::iterator i = subnets.begin(); i != subnets.end(); ++i)
    {
        Subnet *cur = (*i);
...
list<vector<RouteHop> > Vertex::findUniqueRoutes(list<Trail> peerIPs)
{
    // Lists all routes
...
list<IPTableEntry*> Vertex::getAliasCandidates()
{
...
void Vertex::storeAliases(list<Alias*> newAliases)
{
...
bool Vertex::smallerID(Vertex *v1, Vertex *v2)
{
    return v1->getID() < v2->getID();
}

string Vertex::peersToString()
{
    if(peers.size() == 0)
