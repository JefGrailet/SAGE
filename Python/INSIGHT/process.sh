...
# Modes for INSIGHT; no raw graph (i.e. visual of the neighborhood-based DAG) generated by default
nbModes=3
modes[0]="Bip"
modes[1]="BipProjTop"
modes[2]="Trip"
...
datasetPath="/home/jefgrailet/Online repositories/SAGE/Dataset" # TODO: adapt this
...
while [ $i -lt $nbDates ]; do
    ...
    while [ $j -lt $nbModes ]; do
        ...
        if ${withTextDump[$j]} ; then
            python INSIGHT.py ${modes[$j]} "$snapshotPrefix" $outputFilePrefix 10000 > $outputFilePrefix.txt
        else
            python INSIGHT.py ${modes[$j]} "$snapshotPrefix" $outputFilePrefix 10000
        fi
        mv $outputFilePrefix* ./Results/$ASNumber/${datesY[$i]}/${datesM[$i]}/${datesD[$i]}/
        j=`expr $j + 1`
    done
    
    # N.B.: the max number of vertices for graph renders is set to 10000 here but you can use 
    # other values. Keep in mind that for large snapshots with many vertices, the visual rendering 
    # can be very slow (this is due to the spring layout, cf. GraphDrawing.py) in addition to not 
    # being very rewarding (i.e., resulting graphs can be difficult to read). The limit of 10000 
    # vertices in a same visual render makes sure the renders can be computed in at most a few 
    # minutes in addition to remaining readable via suitable software (e.g., Okular).
